...
// Groupes de profils et tarifs classés par âges et par types
const profileGroups = [
    {
...
function renderParticipantsUI() {
    ...
    state.participants.forEach((p, index) => {
        ...
        profileGroups.forEach(group => {
            selectOptions += `<optgroup label="${group.groupLabel}">`;
            group.profiles.forEach(item => {
                const isSelected = p.type === item.key ? 'selected' : '';
                selectOptions += `<option value="${item.key}" ${isSelected}>${item.label}</option>`;
            });
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initSimulatorApp);
} else {
    initSimulatorApp();
}
